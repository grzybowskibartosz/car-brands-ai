#!/usr/bin/env python
"""
car_torch_aug.py – training script for Algerian-Cars brand classifier with richer data augmentation
===============================================================================================
Usage example:
    python car_torch_aug.py --data ./dataset/DATA --aug strong

This revision extends **car_torch.py** with two augmentation presets:
* **basic**  – the original pipeline (default).
* **strong** – adds extra spatial, color and policy-based augmentations (TrivialAugmentWide).

The rest of the logic (metrics CSV, training curves, early stopping, etc.) is unchanged.
"""
...
def parse_args():
    ...
    p.add_argument("--epochs", type=int, default=20, help="epochs for training the classifier head")
    p.add_argument("--fine-epochs", type=int, default=20, help="epochs for full-model fine-tuning")
    ...
    p.add_argument("--aug", choices=["basic", "strong"], default="basic", help="augmentation strength preset")
    return p.parse_args()
...
class ImageDS(Dataset):
    """Generic dataset that supports both TorchVision and Albumentations transforms."""
    ...
    def __getitem__(self, i):
        ...
        # TorchVision transforms use PIL → tensor, Albumentations expects ndarray → dict
        if isinstance(self.tf, transforms.Compose):
            img = self.tf(img)
        else:
            # fallback – just in case a different callable is passed
            img = self.tf(image=np.asarray(img))["image"]
        return img, self.y[i]
...
def get_transforms(sz: int, mode: str = "basic"):
    """Return train & eval transforms according to the selected augmentation *mode*."""

    if mode == "strong":
        train_tf = transforms.Compose([
            transforms.RandomResizedCrop(sz, scale=(0.4, 1.2), ratio=(0.65, 1.5)),
            ...
            transforms.TrivialAugmentWide(num_magnitude_bins=31),  # policy-based Augment → many variants
            transforms.ToTensor(),
            transforms.Normalize(IMAGENET_MEAN, IMAGENET_STD),
            transforms.RandomErasing(p=0.25),
        ])
    else:  # "basic" – the original pipeline
        train_tf = transforms.Compose([
    ...
    # Evaluation/validation transform stays deterministic to ensure repeatability
...
def build_model(num_classes, device):
    ...
    for p in m.features.parameters():
        p.requires_grad = False  # freeze feature extractor for the first stage
...
def plot_curves(tr_losses, val_losses, tr_accs, val_accs, out="train_curves.png"):
    ...
    ax1.plot(ep, tr_losses, label="train loss")
    ax1.plot(ep, val_losses, label="val loss")
    ax1.set_xlabel("epoch")
    ax1.set_ylabel("loss")
    ax2 = ax1.twinx()
    ax2.plot(ep, tr_accs, "--", label="train acc")
...
def main():
    ...
    tf_train, tf_eval = get_transforms(args.img_size, args.aug)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            f"[Head {epoch:02d}/{args.epochs}] loss {tr_loss:.4f}/{val_loss:.4f} acc {tr_acc:.3f}/{val_acc:.3f} lr {lr_now:.2e} ({dt:.1f}s)"
        )
        if val_acc > best_val:
            best_val = val_acc
    ...
    for epoch in range(1, args.fine_epochs + 1):
        ...
        lr_now = sch.get_last_lr()[0]
        print(
            f"[FT   {epoch:02d}/{args.fine_epochs}] loss {tr_loss:.4f}/{val_loss:.4f} acc {tr_acc:.3f}/{val_acc:.3f} lr {lr_now:.2e} ({dt:.1f}s)"
        )
